...
gulp.task('css2', () =>
{
	return gulp.src('stylus/antd.css')
		.pipe(rename(`cts.css`))
		.pipe(gulp.dest(OUTPUT_DIR))
		.pipe(touch());
});

//复制index.html至OUTPUT_DIR目录下；
gulp.task('html', () =>
{
...
let middleware = proxyMiddleware('/file', {
	target: 'https://192.168.0.101:443',
	changeOrigin: true,
	pathRewrite: {
		'^/file': 'https://192.168.0.101:443' //路径重写
	},
	secure: false
});
