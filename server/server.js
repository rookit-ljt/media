...
async function createExpressApp() {
    ...
    expressApp.post('/app', function (req, res) {
            // var url = 'http://47.108.31.106:8091/app/';
        var url = 'http://192.168.0.101:8091/app/';

        var requestData = req.body
            console.log("此处的requestData" + requestData)
            request({
                url: url,
                method: 'post',
                json: true,
    ...
    // expressApp.post('/img', function (req, res) {
    //         var url = 'http://47.108.31.106:8080/upload';
    //         console.log("req"+req.body)
    //         var requestData = req.body
    //         console.log("此处的requestUploadData" + JSON.stringify(requestData))
    //         request({
    //             url: url,
    //             method: 'post',
    //             json: true,
    //             headers: {
    //                 'content-type': 'application/json;multipart/form-data;boundary=----WebKitFormBoundaryXiYlFefaWuZSaPBT'
    //             },
    //             body: requestData
    //         }, function (error, response, body) {
    //             if (!error && response.statusCode == 200) {
...
async function runWebSocketHttpsServer() {
    logger.info('running an WebSocketHttp server...');
    ...
    webSockethttpsServer = https.createServer(tls, expressApp);

    await new Promise((resolve) => {
        webSockethttpsServer.listen("9999", config.https.listenIp, resolve);
    });
}


async function runSocketIoServer() {
    var io = socketIo.listen(webSockethttpsServer);
    //connection
    io.sockets.on('connection', (socket) => {
        socket.on('message', (room, data) => {
            console.log("此处的room,data" + room, data)
        ...
        //whiteBoard
        socket.on('line', (room, clickX, clickY, point) => {
            //房间内所有人
            console.log("此处的白板room" + room);
            socket.to(room).emit('line', room, clickX, clickY, point)
            // io.sockets.emit('line',room, clickX,clickY,point)
        })
